    ...
    <div style={{ backgroundColor: BG, color: TEXT, fontFamily: BODY, minHeight: '100vh' }}>
      ...
      <main style={{ paddingTop: 64 }}>
        ...
        <section style={{ padding: '100px 24px 80px' }}>
          <div style={{ maxWidth: 1024, margin: '0 auto', display: 'flex', flexWrap: 'wrap', gap: 60, alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <div style={{ flex: '0 0 auto' }}>
              <div style={{ width: 280, borderRadius: 24, backgroundColor: SURFACE, border: `1px solid ${BORDER}`, boxShadow: `0 20px 60px rgba(20,20,19,0.08), 0 4px 16px rgba(217,119,87,0.12)`, overflow: 'hidden' }}>
                <div style={{ padding: '16px', backgroundColor: INSET, borderBottom: `1px solid ${BORDER}` }}>
                  <p style={{ fontSize: 9, fontWeight: 700, color: TEXT3, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 4 }}>This week's gut-safe plan</p>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <div style={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: ACCENT }} />
                    <p style={{ fontSize: 12, fontWeight: 600, color: TEXT2 }}>Built from your kitchen</p>
                  </div>
                </div>
                {[
                  { day: 'Mon', meal: 'Lemon herb salmon + rice', tag: 'Low FODMAP' },
                  { day: 'Tue', meal: 'Chickpea & spinach bowl', tag: 'High fiber' },
                  { day: 'Wed', meal: 'Turkey zucchini stir-fry', tag: 'IBS-safe' },
                  { day: 'Thu', meal: 'Oat porridge + blueberries', tag: 'Monash ✓' },
                ].map(({ day, meal, tag }) => (
                  <div key={day} style={{ padding: '12px 16px', borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', gap: 12 }}>
                    <span style={{ fontSize: 10, fontWeight: 700, color: ACCENT, width: 28, flexShrink: 0 }}>{day}</span>
                    <div style={{ flex: 1 }}>
                      <p style={{ fontSize: 12, fontWeight: 600, color: TEXT, marginBottom: 2 }}>{meal}</p>
                      <span style={{ fontSize: 9, fontWeight: 700, color: TEXT3, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{tag}</span>
                    </div>
                  </div>
                ))}
                <div style={{ padding: '12px 16px', backgroundColor: `${ACCENT}0a` }}>
        ...
        <section style={{ padding: '80px 0', borderTop: `1px solid ${BORDER}` }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px,1fr))', borderTop: `1px solid ${BORDER}` }}>
            {([
              { Icon: Utensils, sub: 'From your fridge and pantry', title: 'Weekly meal plans', body: 'Enter what\'s in your kitchen. GutPal generates a week of gut-safe meals built entirely from ingredients you already own. No wasted food, no expensive substitutes, no second-guessing.' },
              { Icon: ShieldCheck, sub: 'At any restaurant', title: 'Eat out without anxiety', body: 'Heading out? GutPal reads the situation and tells you what to order and what to avoid based on your exact triggers. Share your safe picks with the table. Stop dreading the menu.' },
            ] as const).map(({ sub, title, body }, i) => (
              <div key={i} style={{ padding: '48px 32px', borderLeft: i === 1 ? `1px solid ${BORDER}` : 'none' }}>
                <p style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: ACCENT, marginBottom: 12 }}>{sub}</p>
                <h3 style={{ fontFamily: HEADING, fontWeight: 700, fontSize: 22, color: TEXT, marginBottom: 16 }}>{title}</h3>
